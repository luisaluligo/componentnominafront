<template>
  <div>
    <h2 class="encabezado">Editar información del personal</h2>
    <div class="contenedor_info">
      <div class="infoEmpleado">
        <h4>Ingrese el numero de identificación y oprima buscar</h4>
    ...
    <div class="botonesDeAccion">
      <button v-on:click="editarInfoEmpleado">
...
export default {
  ...
  methods: {
    cargarInfoEmpleado: async function () {
      ...
      axios
        ...
        .then((result) => {
          ...
          this.empleado.contrato.fechainicio_con = result.data.contrato.fechainicio_con;
          this.empleado.contrato.fechafin_con = result.data.contrato.fechafin_con;
    ...
    editarInfoEmpleado: async function () {
      if (
        localStorage.getItem("token_access") === null ||
        localStorage.getItem("token_refresh") === null
      ) {
      ...
      axios
        .put(
          `https://componentnominaback.herokuapp.com/editarEmpleado/${codigoEmp}/`,
          this.empleado,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then((result) => {
          ...
          this.$emit("", dataEditarEmpleado);
          alert("Los cambios fueron exitosos");
        })
        .catch((error) => {
          console.log(error);
          if (error.response.status == "401")
